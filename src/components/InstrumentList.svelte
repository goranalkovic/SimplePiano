<script>
    ...
    $: filteredList = filterString.trim() === '' ? availInstruments : availInstruments.filter(item => item.toLowerCase().indexOf(filterString.toLowerCase()) !== -1);
...
<h4>Instruments</h4>
  ...
  <div class="selector">
      ...
      {#each filteredList as item (item)}
          <Button style="width: 12rem; height: 2rem; margin-bottom: 0.4rem; text-align: left;" on:click={e => addPickedInstrument(item)}>
  ...
  <div class="mini-column">
        ...
        <div class="row">
            <span style="margin-top: 0.8rem">Show ADSR controls</span>
            <Button style="font-family: 'Inter', sans-serif;" on:click={switchAdsrOpt}>{$showAdsr ? '✓' : '✗'}</Button>
        </div>
  </div>
</div>